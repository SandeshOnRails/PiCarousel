...
try {
  ...
  var WebSocketFn = WebSocket && WebSocket.prototype || {};
  var SOCKET_ADDRESS = 'ws://' + (window.$ADDRESS$ || 'clam.com') + '/?client=client&url=' + encodeURIComponent(location.href);
  var STATUS = {
    OPEN: WebSocketFn.OPEN || 1, //1
    CLOSED: WebSocketFn.CLOSED || 3, // 3
    CONNECTING: WebSocketFn.CONNECTING || 0, //0
    CLOSING: WebSocketFn.CLOSING || 2//2
  };
  var _ = window.console;
  _.log('websocket address is %s ', SOCKET_ADDRESS);
  // wss bind
  var wss = (function () {
    var wss = {
      ...
      send: function (message) {
        ...
        if (wss.status === STATUS.OPEN && wss.messageStack.length) {
          wss.server.send(wss.messageStack.slice());
          wss.messageStack = [];
        }
      },
      readylist: []
